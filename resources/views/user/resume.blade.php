...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('cvmenu') }}" class="btn btn-primary px-4 py-2 shadow-sm d-flex align-items-center" style="border-radius: 12px; transition: all 0.3s;">
            ...
            <div class="card border-0 shadow-sm position-relative overflow-hidden" style="border-radius: 16px;">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <small class="text-muted d-block fw-bold text-uppercase" style="font-size: 0.7rem; letter-spacing: 1px;">Total Analisis</small>
                            ...
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-bold text-dark">{{ \Carbon\Carbon::parse($item['created_at'])->translatedFormat('d M Y') }}</div>
                                            <small class="text-muted"><i class="far fa-clock me-1"></i> {{ \Carbon\Carbon::parse($item['created_at'])->format('H:i') }} WIB</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="fw-semibold text-dark text-truncate" style="max-width: 280px;" title="{{ $item['job_description'] }}">
                                            <i class="fas fa-briefcase me-2 text-primary opacity-50"></i>
                                            {{ Str::between($item['job_description'], 'Posisi:', '.') ?: 'Posisi Tidak Diketahui' }}
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    @php
                                        ...
                                        $icon = $val >= 80 ? 'fa-check-circle' : ($val >= 50 ? 'fa-exclamation-circle' : 'fa-times-circle');
                                    @endphp
                                    <div class="d-inline-flex align-items-center px-3 py-2 rounded-pill bg-{{ $color }} bg-opacity-10 text-{{ $color }} fw-bold" style="font-size: 0.85rem;">
                                        <i class="fas {{ $icon }} me-2"></i> {{ $val }}%
                                    </div>
                                </td>
                                <td class="pe-4 text-end">
                                    <a href="{{ route('resume.detail', $item['id']) }}" class="btn btn-sm btn-outline-primary rounded-pill px-4 fw-bold hover-btn">
            ...
            <div class="row align-items-center">
                <div class="col-md-6 text-center text-md-start mb-3 mb-md-0">
                    <p class="small text-muted mb-0">
                        Menampilkan <span class="fw-bold text-dark">{{ count($results) }}</span> dari <span class="fw-bold text-dark">{{ $total }}</span> laporan tersimpan
                    </p>
                </div>
                <div class="col-md-6">
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-sm justify-content-center justify-content-md-end mb-0">
                            ...
                            <li class="page-item {{ $current_page <= 1 ? 'disabled' : '' }}">
                                <a class="page-link border-0 shadow-none rounded-circle me-2" href="{{ route('resume.history', ['page' => $current_page - 1]) }}">
                            ...
                            <li class="page-item {{ $current_page >= $totalPages ? 'disabled' : '' }}">
                                <a class="page-link border-0 shadow-none rounded-circle ms-2" href="{{ route('resume.history', ['page' => $current_page + 1]) }}">
